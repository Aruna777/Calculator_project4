...
public partial class CalculatorMainPage : ContentPage
{
    ...
    async void OnCalculate(object sender, EventArgs e)
    {
        if (currentState == 2)
        {
            ...
            string currentCalculation = $"{firstNumber} {mathOperator} {secondNumber}";
            this.CurrentCalculation.Text = currentCalculation;
            this.resultText.Text = result.ToTrimmedString(decimalFormat);

            string calculation = $"{firstNumber} {mathOperator} {secondNumber} = {this.resultText.Text}";

             App.HistoryViewModel.AddtoHistory(calculation);
    ...
    void OnSquareRoot(object sender, EventArgs e)
    {
       ...
       // currentEntry = sq.ToString();
        ...
        CurrentCalculation.Text = sq.ToString();
